...
# %%
words = []
categories = ['numbers', 'algebra', 'games', 'science', 'technology']
...
for cat in categories:
    print(cat)
    closest_word = get_closest_words_from_word(cat, max_n=20)
    temp = pd.DataFrame({
        'category': [cat] * len(closest_word),
        'words': closest_word
    })
...
for i in range(n_rows):
    current_word = df_word_cloud.loc[i, 'words']
...
#%%
ggplot(data=df_word_cloud.sample(25)) + aes(x = 'x', y='y', label = 'words', color = 'category') + geom_text() + labs(title='GloVe Word Embeddings and Categories')
